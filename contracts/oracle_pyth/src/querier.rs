...
pub fn query_price(deps: Deps, env: Env, asset_address: String) -> StdResult<PriceResponse> {
    ...
    let ema_price = price_feed
        .get_ema_price_no_older_than(env.block.time.seconds() as i64, pyth_feeder_config.price_feed_age)
        .ok_or_else(|| ContractError::Std(StdError::not_found("EMA price is not available")))?;

    let feed_time = ema_price.publish_time;
    ...
    let decimal_amount = Decimal256::from_ratio(price, 1) * decimal_places;

    let feed_time_u64: u64 = feed_time.try_into()
