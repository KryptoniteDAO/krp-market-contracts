...
pub fn compute_interest_raw(
    ...
    let mut exchange_rate = compute_exchange_rate_raw(state, atoken_supply, balance);
    ...
    if deposit_rate > target_deposit_rate {
        ...
        let prev_deposits =
            Decimal256::from_uint256(state.prev_atoken_supply * state.prev_exchange_rate);
        ...
        exchange_rate = compute_exchange_rate_raw(state, atoken_supply, balance);
    }
...
pub fn compute_reward(state: &mut State, block_height: u64) {
    ...
    let reward_accrued = passed_blocks * state.kpt_emission_rate;
...
fn assert_max_borrow_factor(
    ...
    {
        let error_msg= format!("state.total_liabilities:{}, borrow_amount:{}, current_balance: {}, state.total_reserves: {}, config.max_borrow_factor: {}", 
                                    state.total_liabilities, borrow_amount, current_balance,
                                    state.total_reserves, config.max_borrow_factor);
        return Err(ContractError::MaxBorrowFactorReached(error_msg));
    }
